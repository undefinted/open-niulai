...
网页端把脚本创作与视频生成分成两个独立环节。用户可使用无需密钥的快速模板，也可临时连接通义千问或 DeepSeek，一次生成三个差异明显的原创脚本候选；选定后还能逐镜头修改动作和台词。随后由 Open NiuLai 的固定风格编译器补齐粗粝低模、僵硬动作、重复台词和延迟揭示等生成约束，再交给 RunningHub 视频实例。

视频端统一使用 RunningHub 生成账户。默认入口是管理员审核并绑定的 MiniMax H3、Seedance 等 AI 应用实例，普通用户只需确认脚本、选择实例和画面来源，不接触 WebAppId、工作流 ID 或节点 ID；自定义工作流保留在高级模式。Cloudflare Pages 正式站点通过同源 Pages Functions 上传素材、提交任务并查询结果。用户提供的脚本和视频 API Key 仅保存在当前浏览器标签页的 `sessionStorage`；生产环境只保存最多七天的匿名任务元数据，用于防重复提交、归属校验和状态恢复。

用户连接一次 RunningHub API Key 后，先确认或修改 Open NiuLai 生成的视频脚本，再选择可用 AI 实例。每次生成前都需明确确认费用，服务随后通过 AI App API 创建任务、查询状态并在结果页播放返回的视频。未配置 HTTPS、未连接账户、实例未在后台绑定或未确认费用时均不会提交任务。MiniMax 官方 API 适配器仅作为后端兼容能力保留，不再出现在默认界面中。

Cloudflare Pages 的构建参数、域名接入和安全边界见 [部署说明](docs/CLOUDFLARE_PAGES.md)。公开试用与正式收费之间的完成边界见 [商业化清单](docs/COMMERCIALIZATION.md)。

脚本 system prompt、全局视觉/动作/负面提示词和内置原创参考图的来源与边界见 [风格配置说明](docs/STYLE_PROFILE.md)。公开检索到的原作截图不会被下载、打包或发送给视频模型。

AI 实例的 `WebAppId` 与参数映射由部署者在服务端配置，不会发送给浏览器。选择“自定义工作流”时，专业用户仍可填写自己的工作流 ID 与节点映射。平台提交任务后持续查询输出，并播放返回的视频文件；关闭页面后仍可从“最近生成任务”恢复查询。配置方法见 [RunningHub 接入说明](docs/RUNNINGHUB.md)。
...
生成台会逐项显示内容、账户、工作流和画面输入是否就绪，并把主按钮切换为当前应完成的下一步。未提交的创意和最近一次制作方案保存在浏览器本地，刷新后可以继续；真实成片完成后可提交匿名评分和主要问题，用于后续调整提示词与工作流。
