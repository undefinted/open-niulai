...
网页端统一使用 RunningHub 生成账户。默认入口是管理员审核并绑定的 MiniMax H3、Seedance 等 AI 应用实例，普通用户只需确认脚本、选择实例和画面来源，不接触 WebAppId、工作流 ID 或节点 ID；自定义工作流保留在高级模式。Cloudflare Pages 正式站点通过同源 Pages Functions 上传素材、提交任务并查询结果。API Key 仅保存在当前浏览器标签页的 `sessionStorage`；生产环境只保存最多七天的匿名任务元数据，用于防重复提交、归属校验和状态恢复。
