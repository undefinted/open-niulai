...
网页端统一使用 RunningHub 生成账户。MiniMax H3、Seedance 和自定义视频能力以 RunningHub 工作流预设呈现，Cloudflare Pages 正式站点通过同源 Pages Functions 上传素材、提交任务并查询结果。API Key 仅保存在当前浏览器标签页的 `sessionStorage`；工作流节点配置与最近 20 条任务保存在浏览器本地，但不包含 API Key、访问密码或上传素材。生产环境只保存最多七天的匿名任务元数据，用于防重复提交、归属校验和状态恢复。

用户连接一次 RunningHub API Key 后，可分别绑定自己的 MiniMax H3、Seedance 或其他工作流。每次生成前都需明确确认费用，服务随后创建任务、查询状态并在结果页播放工作流返回的视频。未配置 HTTPS、未连接账户、未完成工作流绑定或未确认费用时均不会提交任务。MiniMax 官方 API 适配器仅作为后端兼容能力保留，不再出现在默认界面中。

Cloudflare Pages 的构建参数、域名接入和安全边界见 [部署说明](docs/CLOUDFLARE_PAGES.md)。公开试用与正式收费之间的完成边界见 [商业化清单](docs/COMMERCIALIZATION.md)。

用户第一次选择某个预设时填写工作流 ID 与节点映射，之后当前浏览器会记住该预设的非敏感绑定。平台提交 `nodeInfoList` 后持续查询输出，并播放工作流返回的视频文件；关闭页面后仍可从“最近生成任务”恢复查询。使用方法与节点映射见 [RunningHub 接入说明](docs/RUNNINGHUB.md)。
