...
网页端包含统一模型连接中心。MiniMax 和 Runway 预留临时 API Key 连接，可灵与 Seedance 当前提供制作包导出和平台跳转；当供应商向第三方应用开放 OAuth 注册后，可沿用同一连接协议增加 OAuth。临时凭证只保存在服务器内存中，四小时过期且重启即清除。公网 HTTP 环境禁止提交凭证，必须配置 HTTPS 后才能开放连接。

MiniMax H3 已接通站内付费任务闭环：用户连接自己的按量付费 Key，明确确认一次费用后，服务创建一个 H3 V2 任务、按官方建议每 10 秒查询同一任务，成功后下载并在结果页播放 MP4。未配置 HTTPS、未连接账户或未确认费用时均不会提交任务。
