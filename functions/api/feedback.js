...
export async function onRequestPost(context) {
  try {
    ...
    if (!job || job.owner !== session.id) throw new HttpError('未找到该视频任务。', 404, 'job_not_found');
    ...
    await context.env.JOBS.put(`feedback:${session.id}:${provider}:${jobId}`, JSON.stringify(feedback), { expirationTtl: FEEDBACK_TTL });
